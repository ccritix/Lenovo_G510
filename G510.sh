...
echo "$keep_all"
    echo "\n${red}=============================================================="
    echo "${green}${bold}Lenevo Ideapad G510 ${normal}${bold}DSDT/SSDT autopatch script by ${blue}intruder16:${normal}"
    echo "${red}==============================================================${normal}\n"
...
update()
{
    echo "\n${cyan}${bold}Lenevo G510 IdeaPad${normal} - (For Yosemite & up)"
    echo ""
    echo "Visit source : https://github.com/intruder16/G510-OS-X\n"
    echo "Script version \"${green}v$sVersion${normal}\"\n"
    echo "${green}${bold}[---GIT--]${normal}${bold}: Updating to latest G510-OS-X git master....${normal}"
